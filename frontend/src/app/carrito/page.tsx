...
type Plan = 'individual' | 'photopass';
        ...
        {items.map((item) => (
          <div key={item.foto_id} style={{ display: 'flex', alignItems: 'center', gap: '16px', padding: '14px 16px', backgroundColor: '#f8fafc', borderRadius: '14px', border: '1px solid #f0f0f0' }}>
            ...
            <div style={{ textAlign: 'right', flexShrink: 0 }}>
              ...
              <button onClick={() => onQuitar(item.foto_id)}
...
function ResumenPedido({
  ...
  if (plan === 'individual') {
    ...
    lineaPlan = `${items.length} foto${sufijo} × $${PRECIO_INDIVIDUAL}`;
  } else {
    lineaPlan = 'Photopass (evento completo)';
  }
      ...
      <div style={{ backgroundColor: 'white', borderRadius: '20px', padding: '24px', boxShadow: '0 1px 3px rgba(0,0,0,0.06)', border: '1px solid #f0f0f0', marginBottom: '12px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginBottom: '16px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '14px' }}>
            <span style={{ color: '#64748b' }}>{lineaPlan}</span>
...
export default function CarritoPage() {
  ...
  // NOTA: por ahora es solo visual. Para que funcione de verdad falta:
  ...
  const comprar = async () => {
    ...
    try {
      ...
      limpiarCarrito();
      // NOTA: redirigir a PayPhone cuando esté integrado
        ...
        {items.length === 0 ? (
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 380px', gap: '24px', alignItems: 'start' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              <SelectorPlan plan={plan} onSelect={setPlan} items={items} />
              <ListaCarrito items={items} onQuitar={quitar} onLimpiar={limpiarTodo} onAmpliar={setFotoAmpliada} />
            </div>
            ...
            <ResumenPedido
              ...
              setCodigoPromo={setCodigoPromo}
              aplicarCodigo={aplicarCodigo}
